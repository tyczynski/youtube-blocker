...
const videosSelector = 'ytd-rich-grid-video-renderer, ytd-video-renderer, ytd-grid-video-renderer, ytp-videowall-still, ytd-compact-video-renderer';
...
export default class Content {
  ...
  private createQuickBlockButtons(videos: NodeList): void {
    for (let i = 0; i < videos.length; i += 1) {
      ...
      button.innerHTML = '<svg fill="#fff" width="10px" height="10px" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 14 14"><path d="M.293 1.707A1 1 0 111.707.293L7 5.586 12.293.293a1 1 0 111.414 1.414L8.414 7l5.293 5.293a1 1 0 01-1.414 1.414L7 8.414l-5.293 5.293a1 1 0 01-1.414-1.414L5.586 7 .293 1.707z"/></svg>';
  ...
  private blockChannel(video: VideoElement): void {
    const channelNameElement = video.querySelector('ytd-channel-name yt-formatted-string');
  ...
  private includes(name: string, video: HTMLElement, channel: FormattedChannel): void {
    if (Content.transform(channel, name).includes(channel.value)) {
      video.remove();
    }
  }

  private exact(name: string, video: HTMLElement, channel: FormattedChannel): void {
    if (Content.transform(channel, name) === channel.value) {
      video.remove();
    }
  }

  private regex(name: string, video: HTMLElement, channel: FormattedChannel): void {
